...
async function syncGuildRoles(guild, womMemberMap, rankHierarchy, botId) {
	...
	const clannieRole = roles.find(r => r.name.toLowerCase() === 'vegesmite');
	const rankRoles = new Map(
		rankHierarchy
			.map(rank => [rank, roles.find(r => r.name.toLowerCase() === rank.toLowerCase())])
			.filter(([_, role]) => role !== undefined),
	);
	if (!guestRole || !clannieRole) {
		console.error(`Failed finding guest/clannie role in guild: ${guild.name}`);
		return;
	}
...
function processMemberRoles(
	...
	if (
		...
		return handleExcludedMember(member.id, guestRole, clannieRole, memberRoles);
	}
	let displayNames = [
		member.nickname || '',
	...
	if (memberRanks.length === 0) {
		return handleNonMember(member.id, guestRole, clannieRole, memberRoles, rankRoles);
	}
...
function handleExcludedMember(memberId, guestRole, clannieRole, memberRoles) {
	const update = { memberId, rolesToAdd: [], rolesToRemove: [] };
	if (memberRoles.has(guestRole.id)) update.rolesToRemove.push(guestRole);
	if (!memberRoles.has(clannieRole.id)) update.rolesToAdd.push(clannieRole);
	return update.rolesToAdd.length > 0 || update.rolesToRemove.length > 0 ? update : null;
}

function handleNonMember(memberId, guestRole, clannieRole, memberRoles, rankRoles) {
	const update = { memberId, rolesToAdd: [], rolesToRemove: [] };
	if (!memberRoles.has(guestRole.id)) update.rolesToAdd.push(guestRole);
	if (memberRoles.has(clannieRole.id)) update.rolesToRemove.push(clannieRole);
...
function synchronizeMemberRoles(
	...
	// Handle clannie role
	if (!memberRoles.has(clannieRole.id)) {
		update.rolesToAdd.push(clannieRole);
	}
	// Handle rank roles
