...
download_files() {
    ...
    local api_url="https://api.github.com/repos/zroelof/vegesmite-bot/contents/$path?$timestamp"
...
if [ -f "$CONFIG_FILE" ]; then
    ...
    mkdir -p /tmp/vegesmite-backup
    cp "$CONFIG_FILE" /tmp/vegesmite-backup/index.js
fi
...
if [ -f "/tmp/vegesmite-backup/index.js" ]; then
    echo "Restoring config file to $CONFIG_FILE"
    mkdir -p "$BASE_DIRECTORY_PATH/src/config"
    cp "/tmp/vegesmite-backup/index.js" "$CONFIG_FILE"
    rm -rf /tmp/vegesmite-backup
fi
